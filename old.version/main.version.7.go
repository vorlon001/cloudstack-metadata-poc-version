...
func GetServer() (*Server, error) {
	...
	Log.WithFields(logrus.Fields{ "Server": fmt.Sprintf("%#v",server), }).Info("GetServer")
	return &server, nil
}
...
func RequestDump(c *gin.Context, method string) {
        ...
        Log.WithFields(logrus.Fields{ "Request.Method": fmt.Sprintf("%v",c.Request.Method), }).Info(method)
        Log.WithFields(logrus.Fields{ "Request.Header[User-Agent]": fmt.Sprintf("%v",c.Request.Header["User-Agent"]), }).Info(method)
        Log.WithFields(logrus.Fields{ "Request.Host": fmt.Sprintf("%v",c.Request.Host), }).Info(method)
        Log.WithFields(logrus.Fields{ "Request.RemoteAddr": fmt.Sprintf("%v",c.Request.RemoteAddr), }).Info(method)
        Log.WithFields(logrus.Fields{ "Request.RequestURI": fmt.Sprintf("%v",c.Request.RequestURI), }).Info(method)
...
func MethodSubmethodFunction(c *gin.Context) {
	...
	RequestDump(c, "MethodSubmethodFunction")
	...
	responseCode := http.StatusOK
        ...
        case "openstack":
			...
			case "meta_data.json":
				...
				if err != nil {
					Log.WithFields(logrus.Fields{ "Error": fmt.Sprintf("%v", err), }).Info("/openstack/[2018-08-27|latest]/meta_data.json")
