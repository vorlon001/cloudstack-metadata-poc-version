...
func GetMetaData() string {
	RandomSeed, _ := GetRandomSeed()
	return fmt.Sprintf(`{
  "uuid": "CLOUDSTACK-uuid",
  "hostname": "node161.cloud.local",
  "name": "Vm Node161",
  "launch_index": 0,
...
func (s *SingletonOpenstack) GetMetaData(c *gin.Context) string {
	fmt.Printf("???>>>>>>>>>>>>>%v", GetIP(c.Request.RemoteAddr))
	return servers[GetIP(c.Request.RemoteAddr)].MetaData
}
...
func MethodSubmethodFunction(c *gin.Context) {
	...
	response = ""
        ...
        case "openstack":
			...
			case "meta_data.json":
				response = instanceOpenstack.GetMetaData(c)
			case "user_data":
